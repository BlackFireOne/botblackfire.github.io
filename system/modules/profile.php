...
if (($m=='03' && $d>20) || ($m=='04' && $d<21)) return ($lng == "eng") ? "Oven" : "<a class=\"cursor_pointer\" onClick=\"aries.box()\">Овен</a>"; //21.03-20.04
if (($m=='04' && $d>20) || ($m=='05' && $d<22)) return ($lng == "eng") ? "Taurus" : "<a class=\"cursor_pointer\" onClick=\"taurus.box()\">Телец</a>"; //21.04-21.05
if (($m=='05' && $d>21) || ($m=='06' && $d<22)) return ($lng == "eng") ? "Gemini" : "<a class=\"cursor_pointer\" onClick=\"gemini.box()\">Близнецы</a>"; //22.05-21.06
if (($m=='06' && $d>21) || ($m=='07' && $d<23)) return ($lng == "eng") ? "Cancer" : "<a class=\"cursor_pointer\" onClick=\"cancer.box()\">Рак</a>"; //22.06-22.07
if (($m=='07' && $d>22) || ($m=='08' && $d<24)) return ($lng == "eng") ? "Leo" : "<a class=\"cursor_pointer\" onClick=\"leo.box()\">Лев</a>"; //23.07-23.08
if (($m=='08' && $d>23) || ($m=='09' && $d<24)) return ($lng == "eng") ? "Virgo" : "<a class=\"cursor_pointer\" onClick=\"virgo.box()\">Дева</a>"; //24.08-23.09
if (($m=='09' && $d>23) || ($m=='10' && $d<24)) return ($lng == "eng") ? "Libra" : "<a class=\"cursor_pointer\" onClick=\"libra.box()\">Весы</a>"; //24.09-23.10
if (($m=='10' && $d>23) || ($m=='11' && $d<23)) return ($lng == "eng") ? "Sc||pion" : "<a class=\"cursor_pointer\" onClick=\"scorpio.box()\">Скорпион</a>"; //24.10-22.11
if (($m=='11' && $d>22) || ($m=='12' && $d<22)) return ($lng == "eng") ? "Sagittarius" : "<a class=\"cursor_pointer\" onClick=\"sagittarius.box()\">Стрелец</a>"; //23.11-21.12
if (($m=='12' && $d>21) || ($m=='01' && $d<20)) return ($lng == "eng") ? "Capricorn" : "<a class=\"cursor_pointer\" onClick=\"capricorn.box()\">Козерог</a>"; //22.12-20.01
if (($m=='01' && $d>20) || ($m=='02' && $d<19)) return ($lng == "eng") ? "Aquarius" : "<a class=\"cursor_pointer\" onClick=\"aquarius.box()\">Водолей</a>"; //21.01-18.02
if (($m=='02' && $d>18) || ($m=='03' && $d<21)) return ($lng == "eng") ? "Fish" : "<a class=\"cursor_pointer\" onClick=\"pisces.box()\">Рыбы</a>"; //19.02-20.03
...
return null;
            ...
            $znachok = $row['user_znachok'];
                $rowznachok = $db->super_query("SELECT id, img FROM `".PREFIX."_znachok_list` WHERE id
                = '{$znachok}'");
                if($row['user_znachok'] == 0){
            $tpl->set('{user_znachok}', '');
                } else {
            $tpl->set('{user_znachok}', '<img src="../uploads/znachok/'.$rowznachok['img'].'.png" class="znschek">');
            }
...
$tpl->set_block("'\\[privacy-guests\\](.*?)\\[/privacy-guests\\]'si","");
			...
			//Приватность заметок
			if($user_privacy['val_notes'] == 1 OR $user_privacy['val_notes'] == 2 AND $check_friend OR $user_id == $id){
				$tpl->set('[privacy-notes]', '');
				$tpl->set('[/privacy-notes]', '');
			} else
				$tpl->set_block("'\\[privacy-notes\\](.*?)\\[/privacy-notes\\]'si","");
			...
			//################### Подарки ###################//
			if($row['user_gifts']){
				$sql_gifts = $db->super_query("SELECT gift FROM `".PREFIX."_new_gifts` WHERE uid = '{$id}' ORDER by `gdate` DESC LIMIT 0, 5", 1, "user_{$id}/gifts");
				foreach($sql_gifts as $row_gift){
				    $gift_sql = $db->super_query("SELECT img FROM `".PREFIX."_new_gifts_list` WHERE gid = '{$row_gift['gift']}'");
					$gifts .= "<img src=\"/new_gifts/img/gifts/{$gift_sql['img']}.png\" class=\"gift_onepage\" />";
				}
				$tpl->set('[gifts]', '');
				$tpl->set('[/gifts]', '');
				$tpl->set('{gifts}', $gifts);
