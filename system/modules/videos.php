...
if($logged){
	...
	switch($act){
		...
		case "page":
			...
			if(!$CheckBlackList){
				if($last_id){
					...
					//SQL Запрос
					$sql_ = $db->super_query("SELECT id, title, photo, albums, comm_num, add_date, SUBSTRING(descr, 1, 180) AS descr FROM `".PREFIX."_videos` WHERE owner_user_id = '{$get_user_id}' AND id < '{$last_id}' {$sql_privacy} AND public_id = '0' ORDER by `add_date` DESC LIMIT 0, {$limit_vieos}", 1);
					...
					if($sql_){
						...
						foreach($sql_ as $row){
							...
							$tpl->set('[on]', '');
							$tpl->set('[/on]', '');
							$tpl->set_block("'\\[not-on\\](.*?)\\[/not-on\\]'si","");
						} else {
							$tpl->set('[not-on]', '');
							$tpl->set('[/not-on]', '');
		...
		case "addmylist":
			NoAjaxQuery();
			$vid = intval($_POST['vid']);
			$row = $db->super_query("SELECT video, albums, photo, title, descr FROM `".PREFIX."_videos` WHERE id = '{$vid}'");
   ...
   $get_user_id = intval($_GET['get_user_id']);
			...
			$sql_albums = $db->super_query("SELECT id, photo, descr, videos_num FROM `".PREFIX."_videos_albums` WHERE  uid = '{$get_user_id}' ORDER by `id` LIMIT 6", 1);
			...
			if(!$CheckBlackList){
				...
				if($owner){
					...
					if($owner['user_videos_num']){
						...
						//SQL Запрос
						$sql_ = $db->super_query("SELECT id, title, photo, albums, comm_num, add_date, SUBSTRING(descr, 1, 180) AS descr FROM `".PREFIX."_videos` WHERE owner_user_id = '{$get_user_id}' {$sql_privacy} AND public_id = '0' ORDER by `add_date` DESC LIMIT 0, {$limit_vieos}", 1);
