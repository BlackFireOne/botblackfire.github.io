...
if($user_pm_num)
	$user_pm_num = "<div class=\"headm_newac\" style=\"margin-left:14px\">{$user_pm_num}</div>";
else
	$user_pm_num = '';
...
//тех-поддержке +1
 ...
 if($supports == $agent){
 $supports_owner = "<div class=\"headm_newac\"style=\"margin-left:-9px\">{$supports['cnt']}</div>";
 }
 if($supports['cnt'] == 0){
 $supports_owner = '';
 }        	
...
if($user_friends_demands){
	$demands = "<div class=\"headm_newac\" style=\"margin-left:14px\">{$user_friends_demands}</div>";
...
if($user_support)
	$support = "<div class=\"headm_newac\" style=\"margin-left:-9px\">{$user_support}</div>";
...
if($user_info['invties_pub_num']){
	...
	$new_groups = "<div class=\"headm_newag\" style=\"margin-left:90px\">".$user_info['invties_pub_num']."</div>";
...
if($CacheGifts){
	$new_gifts = "<div class=\"headm_newac\">{$CacheGifts}</div>";
	$news_link = '/balance?act=business';
}
...
// Вывод картинки в навигации
if($logged){
if($user_info['user_photo'])
$ava = $config['home_url'].'uploads/users/'.$user_info['user_id'].'/100_'.$user_info['user_photo'];
else
$ava = '/images/no_ava_50.png';
$myphoto_header.='<img src="'.$ava.'" width="30" style="float: left;margin-left: -32px;border-radius: 2px;" />'."\n";
